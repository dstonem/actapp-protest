...
function EventPageFeed({children,event_id}) {
    ...
    return (
        <div>
            ...
            {feedShowing ? <div>{data.map((post,idx) => <Post key={idx} user={post.userName} postImg={post.picurl} postText={post.body} causes={post.causes} action={post.action} />)}<CreatePost event_id={event_id}/></div> : null}
        </div>
    )
}
